// components/Sidebar.jsx — Robinhood-style
...
const styles = `
  ...
  @media (max-width:1023px) {
    ...
    .sb.open { transform:translateX(0); box-shadow:20px 0 60px rgba(0,0,0,0.4); }
  }

  .sb-overlay {
    position:fixed; inset:0; background:rgba(0,0,0,0.55);
    z-index:29; display:none; backdrop-filter:blur(4px);
  }
  @media (max-width:1023px) { .sb-overlay.show { display:block; } }
  ...
  [data-theme="light"] .sb-logo-mark { background:#111; }
  .sb-logo-text { font-size:14px; font-weight:600; color:var(--c-text-primary); letter-spacing:-0.3px; }
  .sb-logo-sub  { font-size:10px; color:var(--c-text-muted); display:block; margin-top:1px; }
  .sb-logo-img  { width:18px; height:18px; filter:invert(1); }
  [data-theme="dark"]  .sb-logo-img { filter:none; }
  ...
  .sb-item {
    ...
    transition:background 80ms; -webkit-tap-highlight-color:transparent;
    position:relative;
  }
  .sb-item:hover  { background:var(--c-elevated); }
  .sb-item.active { background:var(--c-elevated); }

  /* Barra verda esquerra */
  .sb-item-bar {
  ...
  .sb-item.active .sb-item-bar { opacity:1; }

  .sb-item-ico {
    color:var(--c-text-muted); flex-shrink:0;
    display:flex; align-items:center; justify-content:center;
    transition:color 100ms; width:15px; height:15px;
  }
  .sb-item.active .sb-item-ico { color:var(--c-green); }
  .sb-item:hover  .sb-item-ico { color:var(--c-text-secondary); }

  .sb-item-label {
    font-size:13px; font-weight:400; color:var(--c-text-secondary);
    transition:color 100ms;
  }
  .sb-item.active .sb-item-label { color:var(--c-text-primary); font-weight:500; }
  .sb-item:hover  .sb-item-label { color:var(--c-text-primary); }

  .sb-badge {
    margin-left:auto; min-width:16px; height:16px;
    border-radius:8px; background:var(--c-bg-red);
    color:var(--c-red); font-size:9px; font-weight:700;
  ...
  @media (max-width:1023px) {
    .sb-footer { padding-bottom:calc(10px + 60px + env(safe-area-inset-bottom)); }
  }
  ...
  .sb-user-name { font-size:12px; font-weight:500; color:var(--c-text-secondary); }
  .sb-user-sub  { font-size:10px; color:var(--c-text-muted); margin-top:1px; }

  .sb-logout {
    display:flex; align-items:center; gap:7px; padding:8px 9px;
    border-radius:7px; border:none; background:transparent;
    width:100%; cursor:pointer; transition:all 80ms;
    color:var(--c-text-muted); font-family:${FONTS.sans}; font-size:12px;
  }
  .sb-logout:hover { background:var(--c-bg-red); color:var(--c-red); }
`
...
const I = {
  ...
  time:   <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id:'timeline',    label:'Evolució',      icon:I.time   },
    ],
  },
  {
    label: 'Eines',
...
export default function Sidebar({ active, onChange, user, onLogout, isOpen, onClose, activeAlertsCount = 0, theme, onToggleTheme }) {
      ...
      <aside className={`sb${isOpen?' open':''}`}>
        ...
        <div className="sb-logo">
          <div className="sb-logo-mark">
            <img src="/logo_black.png" alt="Cartera" className="sb-logo-img"/>
          </div>
          <div>
            <span className="sb-logo-text">Cartera</span>
            <span className="sb-logo-sub">Gestor financer</span>
          </div>
        </div>
              ...
              {group.items.map(item => (
                ...
                >
                  ...
                  {item.badge && activeAlertsCount > 0 && (
                    <span className="sb-badge">{activeAlertsCount}</span>
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="sb-footer">
          <div className="sb-user">
            ...
            <div style={{flex:1,minWidth:0}}>
              <p className="sb-user-name">{user?.displayName||user?.email?.split('@')[0]||'Usuari'}</p>
              <p className="sb-user-sub">Compte personal</p>
            </div>
          </div>
          <ThemeTogglePill/>
