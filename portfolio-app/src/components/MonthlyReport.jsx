...
const styles = `
  ...
  .mr-hero { text-align:center; padding:28px 20px 20px; }
  .mr-hero-label { font-size:11px; font-weight:400; color:var(--c-text-muted); letter-spacing:0.06em; text-transform:uppercase; margin-bottom:8px; }
  .mr-divider { height:1px; background:var(--c-border); }
  .mr-stats { display:grid; grid-template-columns:repeat(3,1fr); border:1px solid var(--c-border); border-radius:12px; overflow:hidden; }
  .mr-stat { padding:14px 12px; text-align:center; border-right:1px solid var(--c-border); }
  .mr-stat:last-child { border-right:none; }
  .mr-stat-l { font-size:9px; font-weight:500; color:var(--c-text-muted); text-transform:uppercase; letter-spacing:0.10em; margin-bottom:5px; }
  .mr-stat-v { font-size:14px; font-weight:600; font-family:${FONTS.mono}; color:var(--c-text-primary); font-variant-numeric:tabular-nums; letter-spacing:-0.3px; }
  ...
  .mr-hero-m-v { font-size:14px; color:var(--c-text-primary); }
  .mr-hero-m-v.g { color:var(--c-green); }
  .mr-hero-m-v.r { color:var(--c-red); }
  ...
  return (
    <div className="mr">
      ...
      <div className="mr-hero">
        ...
        <p className="mr-hero-title">{fmtEur(total).replace('€','')}<span style={{fontSize:26,opacity:0.4,fontWeight:300}}>€</span></p>
        <p className="mr-hero-sub">Descarrega el resum del portfoli en PDF</p>
      </div>
      <div className="mr-divider"/>
      <div className="mr-stats">
        <div className="mr-stat">
          <p className="mr-stat-l">Guany acumulat</p>
          <p className={`mr-stat-v ${gain>=0?'g':'r'}`}>{gain>=0?'+':''}{fmtEur(gain)}</p>
        </div>
        <div className="mr-stat">
          <p className="mr-stat-l">Capital cost</p>
          <p className="mr-stat-v">{fmtEur(totalCost)}</p>
        </div>
        <div className="mr-stat">
          <p className="mr-stat-l">Posicions</p>
          <p className="mr-stat-v p">{investments.length+savings.length+cryptos.length+(commodities?.length||0)}</p>
        </div>
      </div>
