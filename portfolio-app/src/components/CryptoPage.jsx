  ...
  return (
    <div style={{ fontFamily: "'Geist', sans-serif" }}>
      ...
      {cryptos.length === 0 ? (
        ...
        <div>
          {cryptos.map(c => (
            <CryptoRow key={c.id} c={c} onRemove={onRemove} onEdit={() => setEditing(c)} />
          ))}
        </div>
      )}
...
function CryptoRow({ c, onRemove, onEdit }) {
  ...
  return (
    <div className="row-v2">
      ...
      <div className="row-v2-acts">
        ...
        <button className="row-v2-btn del" onClick={() => onRemove(c.id)} title="Eliminar">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="v2-modal">
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
            {POPULAR.map(p => (
              <button key={p.coinId} onClick={() => selectPopular(p)}
                style={{
        ...
        <div className="v2-space">
          ...
          <Field label="CoinGecko ID">
            <input className="v2-input" style={{ fontFamily: "'Geist Mono', monospace" }} value={form.coinId} onChange={e => set('coinId', e.target.value)} placeholder="bitcoin" />
          </Field>
          <div className="v2-grid2">
        ...
        <div className="v2-modal-footer">
          ...
          <button className="v2-btn-submit" onClick={() => onAdd({ name: form.name.trim(), symbol: form.symbol.toUpperCase(), coinId: form.coinId.toLowerCase(), qty: parseFloat(form.qty)||0, initialValue: parseFloat(form.initialValue)||0, currentPrice: null })}>Afegir</button>
        </div>
      </div>
    </div>
  )
...
function EditCryptoModal({ crypto, onSave, onClose }) {
  const [form, setForm] = useState({ name: crypto.name||'', symbol: crypto.symbol||'', coinId: crypto.coinId||'', qty: crypto.qty||'', initialValue: crypto.initialValue||'' })
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))
